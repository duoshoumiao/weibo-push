...
async def get_weibo_user_latest_posts(uid, count=5, retry=2):  
    ...
    for attempt in range(retry + 1):  
        try:  
            async with aiohttp.ClientSession(headers=headers) as session:  
                async with session.get(url, timeout=10) as resp:    
                    ...
                    if 'application/json' not in resp.headers.get('Content-Type', ''):    
                        ...
                        sv.logger.warning(  
                            f"微博{uid}非JSON响应(尝试{attempt+1}/{retry+1}) - "  
                            f"状态码: {resp.status}, "  
                    ...
                    if data.get('ok') != 1:  
                        sv.logger.warning(  
                            f"微博{uid}获取失败(尝试{attempt+1}/{retry+1}), "  
                            f"API返回: {data}, 重试中"  
                        )  
                        await asyncio.sleep(3)  
                        continue  
            ...
            sv.logger.error(  
                f"微博{uid}请求异常(尝试{attempt+1}/{retry+1}): "  
                f"{type(e).__name__}: {e}"  
            )  
            await asyncio.sleep(3)  # 增加到3秒  
      
    sv.logger.error(f"微博{uid}获取失败（已达最大重试次数{retry+1}次），返回空列表")    
    return []
